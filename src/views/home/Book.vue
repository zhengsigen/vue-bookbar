<template>
  <div class="books" v-show="book.id"  > <!-- style=" overflow: scroll;height: calc(100vh); "  @scroll="scrollHander()" ref="Book"  --> 
            ...
            <div class="book-item">
              ...
              <div class="book-below picture" > <!-- v-for循环 -->
                <div class="book-comment">
                  ...
                  <div v-if="this.total > this.page*5 " @click="uploadMore" style="text-align: center;"> <!-- v-else -->
                    更新评论
  ...
  methods: {
    ...
    uploadMore(){
      ...
      if (Math.ceil(this.total / 5) >= this.page) {
        ...
        this.$http.get("/comment?bookId="+this.$route.params.id + "&page="+this.page).then(res => {
    ...
    //查询书籍评论
    async queryComment(){ 
      let data =  await this.$http.get("/comment?bookId="+this.$route.params.id+"&page="+this.page)
      console.info("查看方法被调用的时候返回的数据：",data)
      if(data.code == 3030){
        console.info("当前没有任何人评论")
      }else{

<template>
  <div
    class="particular"
    v-if="collectionsBooks !== null"
    @scroll="scrollHander()"
    ref="particular"
  >
    <div
      class="particular-evenBetween"
      :style="{'color': 'white', 'background-image': 'linear-gradient(to top,rgba(156, 160, 162, 0.3) 35%,'+collectionsBooks.backgroundColor+' 56%),url('+imgAgency+collectionsBooks.backgroundImage+')','background-size': 'cover, cover','background-position': 'center top, center center'}"
    >
      <header class="header" style="padding:32px 0 80px 0;">
        <h1 class="header-size">{{collectionsBooks.name}}</h1>
        <span class="header-test">{{collectionsBooks.intro}}</span>
      </header>
      <footer class="Flex--evenBetween Flex--alignCenter" style="padding-bottom: 30px;">
        <span>
          <div
            class="text"
            style="vertical-align: middle; font-size: inherit;"
          >{{collectionsBooks.contributorsCount}}人推荐了 {{collectionsBooks.itemsCount}}本书</div>
        </span>
        <div class="flex-avatars">
          <div v-for="(u,index) in users" :key="u.id">
            <span class="flex-avatar" v-if="index < 4">
              <span class="flex-root">
                <img alt :src="u.cover" class="flex-img" />
              </span>
            </span>
          </div>
        </div>
    ...
    <div class="infinite-list-wrapper" style="overflow:auto">
      <div class="ocs" v-for="b in books" :key="b.book.id">
        <div class="oc-item" @click="routerBook(b.book.id)">
          <div class="oc-" style="display: flex;">
            <div class="bookItem" style="margin-left: 16px;border-bottom: none; ">
              <img :src="imgAgency+b.book.cover" alt class="img-par" />
            </div>
            <div class="bookItem-text">
              <div style="font-size: 18px;font-weight: 400;   margin-bottom: 6px;">{{b.book.name}}</div>
              <div class="bookImage-content">{{b.book.author}}</div>
              <div class="bookItem-meta">豆瓣评分 {{b.book.doubanRate}}</div>
            </div>
          </div>
        </div>
        <div class="contribs-wrap">
          <div class="contrib-for-list">
            <p
              class="full-text full-conceal admin"
              :style="conat.includes(b.book.id)?'-webkit-line-clamp: inherit':''"
            >
              {{b.comment}}
              <span class="folding-toggle">
                <button
                  v-show="false"
                  @click="showHidden(b.book.id)"
            ...
            <footer class="contrib-footer">
              <div class="contrib-flex">
                ...
                >
                  <span class="contrib-small" style="flex-shrink: 0;">
                    <img :src="b.user.cover" alt style="    display: block;width: 100%;" />
                  </span>
                  <span class="contrib-text">{{b.user.wxName?b.user.wxName:b.user.name}} 推荐</span>
                </a>
              </div>
              <div class="flex-alignCenter">
                ...
                <div class="interact-actions">
                  ...
                  <div class="action-item" @click="praise(b.id)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="18"
                      height="18"
                    ...
                    >
                      <g
                        :fill="(b.applauded === null | b.applauded === false ? '#FFF': '#D6BA8C')"
                        fill-rule="evenodd"
                        :stroke="(b.applauded === null | b.applauded === false ? '#AAA': '#FFF')"
                        stroke-width="1.2"
                        transform="translate(1 1)"
                      >
                        <path
                    ...
                    <span class="label">{{b.applaud}}</span>
                  </div>
                </div>
              </div>
            </footer>
      ...
      ></div>
      <div v-show="noMore" style="text-align: center; margin: 15px 0;">没有更多了</div>
    </div>
    <div class="AnnoyingButton" @click="routeRecommend">
      <span class="Button">
        <svg
...
export default {
  ...
  methods: {
    ...
    praise(applaudId) {
      this.$http.get("/user/info").then(res => {
        if (res.code === 0) {
          this.$http.post("/applaud/" + applaudId).then(res => {
            if (res.code === 0) {
              console.info(res)
              this.books
                .filter(b => b.id === applaudId)
                .map(b => (b.applauded = res.data.applauded));
              this.books
                .filter(b => b.id === applaudId)
                .map(b => (b.applaud = res.data.applaud));
            }
          });
        }
      });
    ...
    routeRecommend() {
      this.$http.get("/user/info").then(res => {
        if (res.code === 0) {
          this.$router.push({
            path: "/open-collections/" + this.$route.params.id + "/Recommend"
          });
        }
      });
    },
    ...
    async refresh(page) {
      if (Math.ceil(this.total / 5) >= page) {
        ...
        this.$http
          ...
          .then(res => {
            res.data.pageInfo.list.map(d => this.books.push(d));
            if (this.users.length > 5) return;
            res.data.pageInfo.list.map(d => this.users.push(d.user));
            this.loading = false;
          });
      }
      if (Math.ceil(this.total / 5) === page - 1) {
        this.noMore = true;
      }
    },
    showButton(text) {
      var universalNewline = /\r\n|\r|\n/g;
      var line_count = text.split(universalNewline).length;
      return line_count > 4;
    }
  },
  computed: {
  ...
  async created() {
    let data = await this.$http.get("/collection/" + this.$route.params.id);
    this.collectionsBooks = data.data;
    ...
    this.books.push(data.data.frist);
    this.users.push(data.data.frist.user);
    if (data.data.pageInfo === null) return;
    this.total = data.data.pageInfo.total;
    data.data.pageInfo.list.map(d => this.books.push(d));
    data.data.pageInfo.list.map(d => this.users.push(d.user));
    await this.getScorllHeight();
  },
  components: {
    VCount
  }
